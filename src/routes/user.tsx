...
async function verifyJWT(token: { split: (arg0: string) => [any, any, any]; }, context: Context) {
  ...
  return isValid ? JSON.parse(atob(payload)) : null;
}
...
app.post('/login', async (c) => {
  ...
  return c.text(`Your token is: ${token}. Include it with any requests not made from a browser to ensure they are authenticated.`);
});
